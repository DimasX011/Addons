...
function NamePlate:Initialize()
	...
	local function GetQuestDisplayInfo(questID, questLogIndex)
		local text, lootIcon, objectIcon, itemTexture_
		if questID then
			for i = 1, 10 do
		...
		return text, lootIcon, objectIcon, itemTexture_
	end
	...
	function self:NamePlateInfo2VisualizationData(info)
		...
		if questExists and (questLogIndex or questID) and IsQuestWatched(questLogIndex) then
				showIcon = true
				text, lootIcon, objectIcon, itemTexture_ = GetQuestDisplayInfo(questID, questLogIndex)
		else
			...
			if namePlateUnitId then
				...
				if firstQuestInfo then
					text, lootIcon, objectIcon, itemTexture_ = GetQuestDisplayInfo(questID, questLogIndex)
					showIcon = true
					text = text or firstQuestInfo.questName
