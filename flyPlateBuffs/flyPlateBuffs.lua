...
local function ScanUnitBuffs(nameplateID, frame)
	...
	while UnitDebuff(nameplateID,id) do
		local name, _, icon, stack, debufftype, duration, expiration, caster, _, _, spellID = UnitDebuff(nameplateID, id)
	...
	while UnitBuff(nameplateID,id) do
		local name, _, icon, stack, debufftype, duration, expiration, caster, _, _, spellID = UnitBuff(nameplateID, id)
...
function fPB.UpdateAllNameplates(updateOptions)
	for i, p in ipairs(C_NamePlate_GetNamePlates()) do
		UpdateUnitAuras(p.namePlateUnitToken,updateOptions)
	end
end
		...
		if p.Spells then
			...
			for n,s in pairs(p.Spells) do
				...
				if spell then
					...
					temp[spell].name = GetSpellInfo(spellID) or n
				end
			end
			p.Spells = temp
			temp = nil
